...
- **MCP 工具生态**：可配置外部 MCP 服务器（stdio），Agent 通过 `use_mcp_tool` 调用其工具
- 会话内多轮上下文，可随时中断 / 清空
...
- 告警通知：阈值告警（CPU / 内存 / 磁盘 / 负载），支持桌面通知、邮件（SMTP）、钉钉、飞书与通用 Webhook，带冷却防刷
- AI 定时巡检：按计划自动巡检服务器安全（登录失败、异常进程、端口、关键文件等），风险即时通知，一键生成处置建议
...
- [x] MCP 协议支持：Agent 可消费外部 MCP 工具生态
- [x] 监控面板：服务器资源与状态可视化
- [x] 告警通知：阈值告警，桌面通知 / Webhook 渠道
- [x] AI 定时巡检：自动巡检、风险识别、通知与一键处置建议

> 🎉 路线图中的规划功能已全部完成。后续迭代方向：邮件/企业微信等更多通知渠道、巡检命令白名单可视化配置、MCP 服务器双向暴露等。
