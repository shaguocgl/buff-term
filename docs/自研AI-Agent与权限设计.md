...
用于回答"最近怎么样 / 是否变慢 / 磁盘还能撑多久"等趋势类问题，比单次快照更有诊断价值。

- **数据来源**：`host_metrics` 表，由监控面板轮询（`monitor_snapshot`）、agent 会话开始、AI 巡检三个采集点复用写入；同一主机 **60 秒内**已有记录时跳过写入（`insert_metric`），避免监控面板高频轮询把历史记录得过于密集；
- **保留期**：应用启动时清理 90 天前的记录（`prune_metrics`），`window_hours` 上限对齐为 2160 小时（90 天）；
- **`granularity`（分钟 / 小时 / 天）**：
  - `minute`：原始采样点，不聚合，适合看最近几小时的细节波动；
  - `hour`：按小时取均值再分析，适合看当天走势；
  - `day`：按天取均值再分析，适合看长周期趋势 / 容量规划（避免采样密度不均时线性回归被密集时段带偏）；
  - 未显式传入时按 `window_hours` 自动推断：≤6 小时 → minute，≤48 小时 → hour，否则 → day；
- **查询上限**：按窗口大小动态计算（覆盖 60 秒最小写入间隔下窗口内可能出现的全部行数，硬顶 20 万行防护内存），取窗口内**最新**的 N 条（`ORDER BY ts DESC LIMIT` 后反转），避免长窗口叠加密集采样时旧的固定上限会静默丢弃最新数据；
- **回填模型的文本**：聚合/降采样后固定在 30 个点左右外加统计摘要（斜率、外推预测），token 成本与原始采样密度、DB 查询上限无关。
