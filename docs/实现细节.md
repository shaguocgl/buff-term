...
src-tauri/src/         Rust 后端
  ...
  russh.rs             AI 用的 russh 连接池（协议级 exec、known_hosts 校验）
  remote.rs            系统 ssh 批处理执行器（sftp 等场景复用）
  ...
  alert.rs             告警规则与后台评估（桌面通知 / Webhook）
  inspection.rs        AI 定时巡检（只读白名单、风险评级、一键处置建议）
  mcp.rs               自研 MCP stdio 客户端（JSON-RPC）
