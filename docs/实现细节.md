...
src-tauri/src/         Rust 后端
  ...
  remote.rs            系统 ssh 批处理执行器（SFTP、监控采集等场景复用）
...
| 场景 | 通道 | 特点 |
| --- | --- | --- |
| 交互终端 | 系统 OpenSSH + portable-pty | 真 PTY、指纹确认、密码自动填充 |
| AI 工具 / MCP 服务 | russh 连接池（`RusshManager`） | 协议级 exec、known_hosts 校验、按主机复用连接 |
| SFTP / 监控采集 | 系统 ssh / sftp 批处理（`remote::run`） | 单次命令，超时 kill |
...
- 每个会话持有异步读线程，把 PTY 原始字节流通过 `terminal:data` 事件推给前端；EOF 时发 `session:status = exited` 并清理；
- `session_input` 写 PTY、`session_resize` 调整尺寸；
- **密码自动填充**：读线程扫描最近 4KB 输出，命中 `password:` / `passphrase for key` 时写入 `密码 + \r`（只发一次）；UTF-8 按 `char_boundary` 安全截断，避免切到多字节字符。

### 3.3 russh 连接池（russh.rs）
...
- 连接参数显式传入（`-F /dev/null` 风格），忽略用户 ssh 配置，避免 `SendEnv` locale 转发产生远端 setlocale 警告；
...
- `monitor_snapshot` 通过 `remote::run` 在目标主机执行固定脚本，解析出 CPU / 内存 / 磁盘 / 负载 / TOP 进程；
