...
src-tauri/src/         Rust 后端
  ...
  models.rs            数据模型（Host / AiProvider / AiModel / AiRule / McpService / AuditLog / InspectionReport / Remediation）
  db.rs                SQLite：建表、读写
  ...
  alert.rs             通知配置：邮件（SMTP）配置与测试连接
  inspection.rs        AI 只读巡检（含木马 / 挖矿风险采集）、报告持久化、HTML 邮件投递
  ...
  update.rs            GitHub Releases 版本检查
...
src/                   前端（React + TypeScript + xterm.js）
  ...
  components/
    ...
    InspectionPanel.tsx AI 巡检、一键整改、实时进度与历史报告
    HostForm.tsx       新建 / 编辑主机（含密码保存）
    AlertModal.tsx     通知配置（邮件 SMTP 配置与测试）
    Select.tsx         自研下拉框
...
`PRAGMA journal_mode = WAL`、`PRAGMA foreign_keys = ON`，当前 10 张表：
...
| `inspection_reports` | AI 巡检报告与执行状态 | host_id, provider/model, status, risk_level, markdown/html, email_sent, duration_ms |
| `remediations` | 一键整改计划与执行结果（`report_id` 唯一） | host_id, provider/model, intervention, plan_markdown, steps_json, status, error, started_at, finished_at, duration_ms |

### 2.2 凭据存储（credentials.rs）
...
- 命令输出复用 `safety::sanitize` 脱敏与 `util::format_exec_output` 截断，外部调用同样写入审计日志（`permission_mode = mcp`）。
...
## 8. AI 巡检（inspection.rs）

- 点击终端工具栏的巡检入口后，`start_inspection` 会创建一条 `inspection_reports` 记录，并在后台任务中执行巡检；前端可随时取消、查看当前报告或按主机查看最近 30 条历史报告；
- 首先通过 `RusshManager` 执行固定基线脚本（50 秒超时），采集系统、资源、服务、安全、木马 / 挖矿风险与登录事件信息；结果会脱敏并截断至 52KB 后才提交给模型；
- 基线脚本新增 fail2ban 实际配置采集（`jail.local` / `jail.conf` / `jail.d/*` 中的 maxretry、bantime、findtime 等），避免 AI 对已达标项重复建议；
- 木马 / 挖矿风险模块只读采集：可疑进程特征（xmrig、kdevtmpfsi、kinsing 等）、对外连接（`ss -tunp`）、用户与系统 cron、systemd timers、`/tmp` `/dev/shm` `/var/tmp` 可执行文件、最近变动的 SSH `authorized_keys`；
- 模型使用当前启用平台与激活模型，通过 OpenAI 兼容 `chat/completions` 接口生成中文 Markdown 报告。若模型调用 `inspect_exec`，最多允许 7 轮工具调用；
- `inspect_exec` 只接受命令白名单（如 `df`、`free`、`journalctl`、`docker`）；拒绝管道、重定向、命令替换以及写操作关键词。每条动态检查命令限时 20 秒；
- 报告提示词要求整改建议严格基于采集到的真实数值；缺数据时只能标注“未采集，建议人工确认”，不得臆造当前值；木马 / 挖矿判断只报可疑点和证据，建议人工确认；
- 报告按文字关键词归类为低 / 中 / 高 / 未知风险，保存 Markdown 与由 `comrak` 转换的 HTML；若已配置 SMTP 主机与收件人，会尝试发送 HTML 邮件，邮件失败不会使巡检任务失败；
- 任务通过 `inspection:progress`、`inspection:done`、`inspection:error` 事件向前端报告阶段与结果；取消使用原子标志，在采集、分析与工具调用间检查。

### 8.1 一键整改（remediation.rs）

- 巡检报告成功后，前端「一键整改」可输入整改干预意见；`start_remediation_planning` 创建一条 `remediations` 记录（`report_id` 唯一），并后台调用当前激活 AI 生成结构化整改步骤；
- 规划阶段不执行任何命令，仅输出 JSON：`{ summary, steps: [{ description, command, timeout_secs }] }`；解析失败则标记 `failed` 并允许重试；
- 用户在 `plan_ready` 状态可编辑 / 删除步骤；确认后 `execute_remediation` 按顺序执行每条命令，单步超时默认 60 秒（限制 5–600 秒），输出脱敏并截断至 8000 字符；
- 每条整改命令都会写入 `audit_logs`（`tool_name=remediation`、`approval=confirmed`），任一步失败立即停止并置 `failed`，步骤之间检查取消标志；
- 失败或取消后可通过 `retry_remediation` 重新执行；整改成功或失败后，若已配置 SMTP，会自动发送包含步骤与结果的 HTML 邮件；
...
### 10.1 应用内版本检查（update.rs）

- 当前版本直接取 Rust 编译期 `CARGO_PKG_VERSION`；
- 「检查更新」请求 `https://api.github.com/repos/shaguocgl/keywisp-agent-ops/releases/latest`，仅比较最新正式 Release 的 tag 与当前版本；
- tag 的可选 `v` 前缀会被忽略，比较采用主版本 / 次版本 / 修订版本的数值顺序；不存在 Release 时返回“尚未发布”，网络或 GitHub 非成功响应会返回可见错误；
- 有更新时前端显示下载版本号，用户再次点击会打开该 Release 页面。当前实现只做检测与下载引导，**不包含自动下载、安装或重启**。

### 10.2 GitHub Actions 自动发布

`.github/workflows/release.yml` 在推送 `v*` tag 或手动触发时运行：

1. 在 macOS runner 构建 Intel + Apple Silicon 通用 DMG；
2. 在 Windows runner 构建 x64 NSIS 安装包；
3. 用 `tauri-apps/tauri-action` 创建 / 更新 `v__VERSION__` 对应的正式 GitHub Release 并上传安装包；
4. 若由 tag 触发，工作流要求 tag、`package.json`、`src-tauri/tauri.conf.json` 与 `src-tauri/Cargo.toml` 版本严格一致，例如都为 `1.0.0` 时只能使用 `v1.0.0`。

该工作流不配置 Apple / Windows 代码签名，也不配置 Tauri Updater 私钥；因此可自动构建和发布，但下载后的系统安全提示与“应用内自动安装更新”需在后续签名流程中解决。

## 11. 前端实现

### 11.1 终端：先监听后连接（TerminalView.tsx）
...
- 挂载时调用 `get_history(hostId)`，把后端扁平历史还原成“用户 + 助手（含工具卡片）”展示；清空对话会先调用 `agent_reset(sessionId, hostId)` 停止后台循环，再忽略迟到的流式事件。

### 11.3 MCP 服务弹窗（McpServiceModal.tsx）
...
### 11.4 自研下拉框（Select.tsx）

- 按钮 + 绝对定位菜单，箭头旋转动画、选项选中打勾；
- 键盘支持 `↑`/`↓`/`Enter`/`Esc`，点击外部关闭；
- `select-up` 类使聊天底部的下拉向上弹出，避免被窗口底部截断。

### 11.5 侧边栏版本入口（App.tsx）

- 应用启动时调用 `get_app_version` 显示本机版本，不依赖网络；
- 「检查更新」期间禁用按钮，成功后显示“已是最新”或“下载更新 vX.Y.Z”；发现更新后点击按钮会跳转到 GitHub Release；
- 版本检查异常通过现有 Toast 呈现，不影响主机、会话和 AI 功能。
...
| Agent | `agent_chat` `agent_approve` `agent_cancel` `agent_reset` `get_history` |
| 审计 | `list_audit_logs` |
| SFTP | `sftp_list` `sftp_download` `sftp_upload` `sftp_delete` `sftp_mkdir` `sftp_rename` |
| 监控 | `monitor_snapshot` |
| AI 巡检 | `start_inspection` `get_inspection_report` `list_inspection_reports` `delete_inspection_report` `cancel_inspection` |
| AI 整改 | `start_remediation_planning` `get_remediation` `execute_remediation` `cancel_remediation` `retry_remediation` |
| 通知配置 | `get_alert_settings` `save_alert_settings` `test_alert_settings` |
| MCP 服务 | `get_mcp_service` `save_mcp_service` `rotate_mcp_token` `mcp_approve` `list_mcp_rules` `add_mcp_rule` `delete_mcp_rule` |
| 版本 | `get_app_version` `check_for_update` |
...
| `inspection:progress` | report_id, phase, message | 巡检采集 / 分析 / 动态检查 / 渲染 / 邮件进度 |
| `inspection:done` | report_id, status | 巡检成功或取消完成 |
| `inspection:error` | report_id, message | 巡检失败 |
| `remediation:progress` | remediation_id, phase, message, step_index, total | 整改规划 / 逐步执行进度 |
| `remediation:done` | remediation_id, status | 整改规划完成 / 执行成功或取消 |
| `remediation:error` | remediation_id, message | 整改规划或执行失败 |
...
- AI 巡检的动态命令使用白名单与特征拦截，不应将其视为沙箱；应仅对可信主机开启巡检，并审阅生成报告中的建议；
- 一键整改会在目标主机执行写操作，必须由用户在确认计划后触发，并逐条审计；执行前应人工审阅 AI 生成的命令，避免误执行破坏性操作；
- 版本检查只访问 GitHub 公共 Release 元数据，不上传主机、凭据、审计日志或 AI 配置；
