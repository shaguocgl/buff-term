...
src-tauri/src/         Rust 后端
  ...
  inspection.rs        AI 只读巡检、报告持久化、HTML 邮件投递
...
src/                   前端（React + TypeScript + xterm.js）
  ...
  components/
    ...
    InspectionPanel.tsx AI 巡检、实时进度与历史报告
...
`PRAGMA journal_mode = WAL`、`PRAGMA foreign_keys = ON`，当前 9 张表：
...
- 首先通过 `RusshManager` 执行固定基线脚本（50 秒超时），采集系统、资源、服务、安全和登录事件信息；结果会脱敏并截断至 32KB 后才提交给模型；
...
4. 若由 tag 触发，工作流要求 tag、`package.json`、`src-tauri/tauri.conf.json` 与 `src-tauri/Cargo.toml` 版本严格一致，例如都为 `0.1.1` 时只能使用 `v0.1.1`。
